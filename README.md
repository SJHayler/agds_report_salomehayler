...
For the Digital Soil Mapping exercise, the covariates constitute an extremely large dataset. Because the TIFF files are very large, this folder could not be uploaded to GitHub. If you want to access the original covariates for preprocessing, please check the data_raw folder in the README, which contains a link to the repository for the soil data. This allows downloading the full dataset to your personal computer. However, downloading the original covariates is *not necessary* to run the R Markdown script, as it is already linked to the preprocessed covariates in the /data folder.
